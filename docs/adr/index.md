...
- [0006 Architecture boundaries the model is asked to flag](0006-decision-required-boundaries.md): the prompt enumerates the same architecture boundaries as the project's own decision policy, so a queue or worker is flagged rather than read as ungoverned — **proposed**.
