...
- [0003 Prompt-injection threat model and unforgeable document delimiters](0003-prompt-injection-threat-model.md): the diff is untrusted, so a per-request secret nonce fences trusted documents from it and a forged citation is still rejected — **proposed**.
