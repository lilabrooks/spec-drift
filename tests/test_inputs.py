...
from spec_drift.inputs.mapping import parse_mappings, resolve_governing_docs
...
def test_env_binary_and_ignored_are_excluded(mixed) -> None:  # type: ignore[no-untyped-def]
...
def test_path_escaping_the_root_is_excluded() -> None:
    ...
    reason = classify_exclusion(
        Path("/repo"), change, is_binary=lambda *_: False, is_ignored=lambda *_: False
    )
    assert reason is ExclusionReason.OUTSIDE_ROOT
...
def test_partition_keeps_ordinary_paths() -> None:
    ...
    kept, excluded = partition(
        Path("/repo"), changes, is_binary=lambda *_: False, is_ignored=lambda *_: False
    )
